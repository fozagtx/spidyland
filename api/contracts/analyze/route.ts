...
import { mistral } from "@/lib/utils/openAIClient";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Analyze with Mistral AI using Vercel AI SDK
    const { object } = await generateObject({
      model: mistral('mistral-large-latest'),
      prompt: `${ANALYSIS_PROMPT} ${textContent}`,
      schema: {
        type: 'object',
        properties: {
          amounts: {
            ...
            items: {
              ...
              required: ['amount', 'currency', 'for', 'location']
            }
          },
          tasks: {
            type: 'array',
